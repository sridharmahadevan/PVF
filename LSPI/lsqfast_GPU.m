function [w, A, b] = lsqfast_GPU(samples, policy, new_policy, firsttime)
...
gPiPhihat = gpuArray(PiPhihat); 
gRhat = gpuArray(Rhat); 

ggamma = gpuArray(new_policy.discount); 

%%% Compute the matrices A and b
%gA = gPhihat * (gPhihat' - new_policy.discount * gPiPhihat);
gA = gPhihat * (gPhihat' - ggamma * gPiPhihat);
gb = gPhihat * gRhat;
